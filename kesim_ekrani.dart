...
class KesimEkrani extends StatefulWidget {
  final String isId;
  const KesimEkrani({super.key, required this.isId});
...
class _KesimEkraniState extends State<KesimEkrani> {
  ...
  String _isAdi = 'İş Adı';
  List<Pusula> _pusulalar = [];
  int _seciliIndex = 0;
  ...
  Future<void> _yukle() async {
    final is_ = await _db.isGetir(widget.isId);
    if (is_ == null || !mounted) return;
    setState(() {
      _isAdi = is_.isAdi;
      _pusulalar = is_.pusulalar;
      _yukleniyor = false;
    });
  }

  void _kaydet() {
    _db.pusulalariGuncelle(widget.isId, _pusulalar);
  }
      ...
      builder: (context) {
        return Padding(
          ...
          child: Column(
            ...
            children: _pusulalar.asMap().entries.map((e) {
              return ListTile(
                title: Text(e.value.isim, style: const TextStyle(color: OrjandaRenkleri.yazi)),
                onTap: () {
                  setState(() => _seciliIndex = e.key);
  ...
  void _isiBitir() {
    _db.isiBitir(widget.isId, DateTime.now());
