...
class VeritabaniServisi {
  ...
  ///
  /// Not: sıralama bilinçli olarak Firestore sorgusunda değil, sonuç
  /// geldikten sonra burada yapılıyor — iki eşitlik filtresi + sıralama
  /// birleşimi Firestore'da özel bir "composite index" gerektirir; bunu
  /// burada yaparak o kuruluma hiç ihtiyaç bırakmıyoruz.
  Stream<List<IsKaydi>> tamamlananIsleriDinle(String kullaniciId) {
    return _isler
        ...
        .snapshots()
        .map((sonuc) {
          final liste = sonuc.docs.map((d) => IsKaydi.fromDoc(d)).toList();
          liste.sort((a, b) {
            final ab = a.bitisTarihi ?? a.baslamaTarihi;
            final bb = b.bitisTarihi ?? b.baslamaTarihi;
